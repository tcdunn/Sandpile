...
FILE *dungeon_file(char *version){
  char *home = getenv("HOME");
  char *path;
  path = malloc((strlen(home) + strlen(FILENAME) + 2 + 1) * sizeof(char));
  strcpy(path, home);
  strcat(path, FILENAME);
  
  FILE *f = fopen(path, version);
  if(!f){
    fprintf(stderr, "Critical Error: Incorrect file input");
    free(path);
    exit(-1);
  }
  free(path);
...
int main(int argc, char *argv[])
{
  ...
  //implementation without seed input from user
  int load = 0;
  int save = 0;
  ...
  if(load == 1){
    //Call a load Dungeon function that generates the dungeon by reading from a file
    load(&d);
  }else{
    //Generate normal dungeon using seed
  ...
  if(save == 1){
    //call a save dungeon method that writes to a file
    save(&d);
  }
